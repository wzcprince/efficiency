...
def replace_all_syscall(item_name, item_path, content):
    ...
    while match1:
        ...
        if begin_str.endswith("/*") or begin_str.endswith("//"):
            ...
            print "{:<46} {:<32} commented".format(item_path, match1.group(2))
            ...
            print "{:<46} long {}{}".format(item_path, sys_call_name, final_para_str)
            ...
            #print start_pos, " ", str(original_length), " ", str(len(content))
        else:
            print "{} {}".format(item_path, match1.group(0))
            pass
...
def replace_LIST_HEAD(content):
    ...
    while match1:
        ...
        print item_path, " start_pos: ", start_pos
...
def replace_static_const_struct(content):
    ...
    while match1:
        start_pos = match1.end()
        print str(start_pos) + ":", match1.group(0)
        if match1.group(1):
            # 即是以/*开头已经注释过的行，不需要再重复注释啦
            print "commented"
            pass
